...
$cmakeConfigRoot = Join-Path $BuildPath "lib\cmake"
$cmakeSearchRoot = Join-Path $BuildPath "lib"
...
if (-not (Test-Path -LiteralPath $cmakeConfigRoot)) {
    throw "Pre-built LLVM CMake files were not found under '$cmakeConfigRoot'."
}
...
Get-ChildItem -LiteralPath $cmakeSearchRoot -Recurse -Filter *.cmake | ForEach-Object {
