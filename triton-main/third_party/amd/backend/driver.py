...
def _get_path_to_hip_runtime_dylib():
    ...
    # On Windows, LoadLibrary needs a .dll, not a .lib import library.
    if os.name == 'nt':
        dll_path = os.path.join(os.environ['HIP_PATH'], 'bin', 'amdhip64.dll')
        if os.path.exists(dll_path):
            return dll_path.replace('\\', '\\\\')
...
#define QUERY_EACH_FN(hipSymbolName, ...)                                     \\
  *(void **)&hipSymbolTable.hipSymbolName = GetProcAddress(lib, #hipSymbolName);       \\
                                                          \\
  if (*(void **)&hipSymbolTable.hipSymbolName == NULL) {{                                                               \\
...
class HIPDriver(GPUDriver):
    ...
    def is_active():
        try:
            ...
            if cc == (8, 8):
              return True
            if 'ZLUDA' in torch.cuda.get_device_name():
              return True
