...
def update_symlink(link_path, source_path):
    ...
    if link_path.is_symlink():
        ...
        # Python 3.12+ on Windows: shutil.rmtree fails on junctions ("Cannot call rmtree on a symbolic link")
        if hasattr(os.path, 'isjunction') and os.path.isjunction(link_path):
            os.rmdir(link_path)  # Junctions can be cleanly removed with rmdir
        else:
    ...
    try:
        ...
        if platform.system() == "Windows" and source_path.is_dir():
            # Use directory junction (mklink /J) which doesn't require admin
            print(f"symlink failed, falling back to junction (mklink /J): {link_path} -> {source_path}", file=sys.stderr)
            try:
                subprocess.run(
                ...
                print(f"junction failed too, skipping symlink creation (build may still work): {link_path}", file=sys.stderr)
        else:
            print(f"symlink failed, falling back to copytree: {link_path} -> {source_path}", file=sys.stderr)
...
def download_and_copy(name, src_func, dst_path, variable, version, url_func):
    ...
    arch = {"AMD64": "64", "arm64": "sbsa", "aarch64": "sbsa"}.get(arch, arch)
