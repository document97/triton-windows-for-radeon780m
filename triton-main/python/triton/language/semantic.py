...
def dot(lhs: tl.tensor, rhs: tl.tensor, acc: tl.tensor, input_precision: Optional[str], max_num_imprecise_acc: int,
    ...
    if lhs.dtype.is_fp8() and rhs.dtype.is_fp8():
        ...
        assert lhs.dtype in (tl.int4, tl.uint4, tl.int8, tl.uint8, tl.float16, tl.bfloat16,
                             tl.float32), f"Unsupported lhs dtype {lhs.dtype}"
        assert rhs.dtype in (tl.int4, tl.uint4, tl.int8, tl.uint8, tl.float16, tl.bfloat16,
    ...
    if lhs.type.scalar.is_int():
        assert lhs.type.scalar in (tl.int4, tl.uint4, tl.int8, tl.uint8), \
            "only int4/int8 integer dot operands are supported"
